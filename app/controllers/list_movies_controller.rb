class ListMoviesController < ApplicationController
  ...
  before_action :set_user, only: [:create, :create_watched, :create_want, :destroy, :create_watched_from_want]
  before_action :logged_in_user, only: [:create, :create_watched, :create_want, :destroy, :create_watched_from_want]
  ...
  def correct_user
    list = @user.movielists.find_by(id: params[:movielist_id])
